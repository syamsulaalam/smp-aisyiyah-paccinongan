    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            if (isEditMode) {
                ...
                // Kita wajib pakai FormData agar file terkirim
                const submitData = new FormData();
                
                // Masukkan field sesuai nama yang diminta Backend (judul, isi)
                ...
                if (imageSource === 'upload' && selectedFile) {
                    ...
                    // Kirim URL
                    submitData.append('gambar', formData.imageUrl);
                }

                // Kirim ke API
                await api.addNews(submitData);
            }
            onSave();
        } catch (error) {
            console.error('Failed to save article', error);
            alert('Gagal menyimpan berita. Pastikan backend menyala.');
        } finally {
            setIsSaving(false);
        }
    };
                    ...
                    <div className="p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="content" id="content" value={formData.content} onChange={handleChange} required rows={8} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                            </div>
                            ...
                            {imageSource === 'url' ? (
                                <div>
                                    ...
                                    <input type="url" name="imageUrl" id="imageUrl" value={formData.imageUrl} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
                                </div>
                            ) : (
                                <div>
                                    <label htmlFor="imageUpload" className="block text-sm font-medium text-gray-700">Pilih File Gambar</label>
                                    <input type="file" name="imageUpload" id="imageUpload" accept="image/*" onChange={handleFileChange} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                                </div>
                            )}
                            ...
                            {formData.imageUrl && (
                             ...
                             <div>
                                ...
                                <select name="status" id="status" value={formData.status} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
...
const ManageNewsPage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto">
            {isModalOpen && (
                <NewsFormModal 
                    article={editingArticle || emptyArticle}
                                ...
                                <tr key={article.id}>
                                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <div className="flex items-center">
                                            {article.imageUrl && (
                                                <img src={article.imageUrl} alt="" className="w-10 h-10 rounded-full mr-3 object-cover"/>
                                            )}
